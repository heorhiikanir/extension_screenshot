...
async function OnLoad() {
	...
	if (savedRecordings && savedRecordings?.length > 0) {
		...
		for (var [index, recording] of savedRecordings.entries()) {
			...
			var recordedAudio = $(`<option>Audio ${index + 1}&nbsp;&nbsp;(${durations[index]} seconds) <br> Comment: ${element} </option>`);

			$("#slcRecordings").append(recordedAudio);

			var updatedContainer = $(`
          <div class="comment-container">
            <div class="comment-text">${comments[comments?.length - 1]}</div>
            <button class="edit-button">Edit</button>
			<button class="delete-button">Delete</button>
          </div>
        `);
			$("#audioTable").append(updatedContainer)
...
async function Record() {
	...
	chrome.tabCapture.capture({ audio: true, video: false }, (stream) => {
		...
		recorder.onstop = async (e) => {
			...
			var audio = $(`
                <option>
                    Audio ${$("#slcRecordings option")?.length + 1}&nbsp;&nbsp;(${c && c ? c : duration} seconds)
                    <br>
                    Comment: ${comment}
                </option>`
			);
...
$(document)?.on("click", ".edit-button", editComment)
// $(document).on("click", ".delete-button", deleteAudio)
$(document)?.on("click", ".delete-button", deleteAudio);

async function deleteAudio() {

	var audioContainer = $(this).closest(".comment-container");
	var audioIndex = $(".comment-container").index(audioContainer);
...
async function editComment() {
	...
	var commentIndex = $(".comment-container").index(commentContainer);
	...
	if (editedCommentText !== null) {
		...
		audioOption.html(
			`Audio ${commentIndex + 1}&nbsp;&nbsp;(${durations[commentIndex]
			} seconds)<br>Comment: ${editedCommentText}`
		);
	}
